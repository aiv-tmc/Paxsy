...
static int process_one_file(const char *filename, const char *output_asm,
    ...
    printf("[DEBUG] Tokenization done\n"); fflush(stdout);

    if (!errhandler__has_errors()) {
        printf("[DEBUG] Parsing...\n"); fflush(stdout);
        ast = parse(lexer->tokens, lexer->token_count);
        printf("[DEBUG] Parsing done\n"); fflush(stdout);
    }

    if (*semantic_ctx && ast && !errhandler__has_errors()) {
        printf("[DEBUG] Semantic analysis...\n"); fflush(stdout);
        if (flags & F_WEXTRA) { semantic__set_extra_warnings(*semantic_ctx, true); }
        if (!semantic__analyze(ast, resolver)) { err = 1; }
        printf("[DEBUG] Semantic done\n"); fflush(stdout);
        if (!errhandler__has_errors()) {
            printf("[DEBUG] Optimizing...\n"); fflush(stdout);
            struct Scope *global_scope = semantic__get_global_scope(*semantic_ctx);
            if (!optimizer((ASTNode*)ast, global_scope)) { err = 1; }
            printf("[DEBUG] Optimization done\n"); fflush(stdout);
        }
    }
