...
typedef struct {
    ...
    size_t      data_size;
    uint32_t    alignment;
    Build32_Word  name_offset;
    size_t      file_offset;
    /* Additional fields required for ELF section headers */
    Build32_Word  sh_link;
    Build32_Word  sh_info;
    Build32_Word  sh_entsize;
} SectionInfo;
...
static int write_section_header(FILE *f, const SectionInfo *sec) {
    ...
    if (write_le32(f, 0) != 0) return -1;          /* sh_addr – always 0 for relocatable */
    if (write_le32(f, (uint32_t)sec->file_offset) != 0) return -1;
    if (write_le32(f, (uint32_t)sec->data_size) != 0) return -1;
    if (write_le32(f, sec->sh_link) != 0) return -1;
    if (write_le32(f, sec->sh_info) != 0) return -1;
    if (write_le32(f, sec->alignment) != 0) return -1;
    if (write_le32(f, sec->sh_entsize) != 0) return -1;
    return 0;
}
...
uint8_t build__add_section(BuildObjectWriter *w, SectionType type,
    ...
    /* Initialize additional header fields */
    sec->sh_link = 0;
    sec->sh_info = 0;
    sec->sh_entsize = 0;

    w->section_count++;
    return (uint8_t)(w->section_count);
}
...
int build__finalize(BuildObjectWriter *w) {
    ...
    int shstrtab_index = 1 + user_count;
    int symtab_index   = 1 + user_count + 1;
    int strtab_index   = 1 + user_count + 2;
    ...
    special[1].sh_entsize = sizeof(Build32_Sym);
    ...
    current_offset += special[0].data_size;
    special[1].file_offset = current_offset;
    current_offset += special[1].data_size;
    special[2].file_offset = current_offset;
    current_offset += special[2].data_size;
    size_t shoff = current_offset;

    /* Determine entry point value */
    ...
    if (write_le16(w->file, ET_REL) != 0) goto fail;
    if (write_le16(w->file, EM_386) != 0) goto fail;
    if (write_le32(w->file, EV_CURRENT) != 0) goto fail;
    if (write_le32(w->file, entry) != 0) goto fail;
    if (write_le32(w->file, 0) != 0) goto fail;
    if (write_le32(w->file, (uint32_t)shoff) != 0) goto fail;
    if (write_le32(w->file, 0) != 0) goto fail;
    if (write_le16(w->file, sizeof(Build32_Ehdr)) != 0) goto fail;
    if (write_le16(w->file, 0) != 0) goto fail;
    if (write_le16(w->file, 0) != 0) goto fail;
    if (write_le16(w->file, sizeof(Build32_Shdr)) != 0) goto fail;
    if (write_le16(w->file, (Build32_Half)total_sections) != 0) goto fail;
    if (write_le16(w->file, (Build32_Half)shstrtab_index) != 0) goto fail;

    /* Write section data */
    if (write_section_data(w->file, w->sections, user_count) != 0) goto fail;
    if (write_shstrtab(w->file, w->sections, user_count) != 0) goto fail;
    for (int i = 0; i < w->symbol_count; i++) {
    ...
    {
        int first_global = 0;
        while (first_global < w->symbol_count && w->symbols[first_global].bind == STB_LOCAL)
            first_global++;
