 ...
 *  (i.e. after "0x" there is a dot or a 'p' later).
 */
...
static Token parse_decimal_number(Lexer *lexer, uint16_t start_line,
    ...
    if (has_int) {
        ...
        if (buffer_append_range(&buf, &cap, &len, src + int_start,
                                int_end - int_start) != 0) {
    ...
    if (!error && lexer->position < lexer->source_length &&
        ...
        if (buffer_append_range(&buf, &cap, &len, src + frac_start,
        ...
        while (!error && lexer->position < lexer->source_length &&
            ...
            if (buffer_append_range(&buf, &cap, &len, src + group_start,
    ...
    /* Exponent (only for floats) */
    if (!error && is_float && lexer->position < lexer->source_length &&
        ...
        if (!consume_exponent(lexer)) {
            ...
            if (buffer_append_range(&buf, &cap, &len, src + exp_start,
                                    exp_end - exp_start) != 0) {
                error = true;
            }
        }
 ...
 *  Returns a TOKEN_NUMBER_LITERAL or TOKEN_ERRORCODE.
 */
static Token parse_hex_float(Lexer *lexer, uint16_t start_line,
                             uint16_t start_col) {
    const char *src = lexer->source;
    bool error = false;
    ...
    while (lexer->position < lexer->source_length) {
        char c = src[lexer->position];
        if (isxdigit((unsigned char)c) && c >= 'A' && c <= 'F') {
 ...
 *  Returns a TOKEN_NUMBER_LITERAL or TOKEN_ERRORCODE.
 */
static Token parse_non_decimal_integer(Lexer *lexer, int base,
    ...
    if (buffer_append_range(&buf, &cap, &len, src + digit_start,
...
Token literal__parse_number(Lexer *lexer) {
    ...
    if (lexer->position < lexer->source_length &&
        ...
        if (next == 'b') {
            lexer->position += 2;
            lexer->column += 2;
            return parse_non_decimal_integer(lexer, 2, start_line, start_col);
        } else if (next == 'o') {
            lexer->position += 2;
            lexer->column += 2;
            return parse_non_decimal_integer(lexer, 8, start_line, start_col);
        } else if (next == 'x') {
            lexer->position += 2;
            lexer->column += 2;
            if (is_hex_float_candidate(lexer)) {
                return parse_hex_float(lexer, start_line, start_col);
            }
            return parse_non_decimal_integer(lexer, 16, start_line, start_col);
        }
    }

    /* Decimal (integer or floating‑point) */
    return parse_decimal_number(lexer, start_line, start_col);
}
