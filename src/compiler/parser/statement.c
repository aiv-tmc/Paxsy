...
ASTNode* parse_block(Parser* p) {
    if (!match(p, TOKEN_LBRACE)) {
        errhandler__report_error(p->line, p->column, "parser", "Expected '{'");
        p->panic_mode = true;
        return NULL;
    }
...
ASTNode* parse_if_stmt(Parser* p) {
    ...
    while (match(p, TOKEN_ELSE)) {
        if (peek(p) == TOKEN_IF) {
            ...
            append_child(p, (void***)&stmt->else_if_conditions, &stmt->else_if_count, econd);
            append_child(p, (void***)&stmt->else_if_branches, &stmt->else_if_count, ebody);
        } else {
            ASTNode* ebody = parse_block(p);
            if (!ebody) {
                errhandler__report_error(p->line, p->column, "parser",
...
/*
 * Parses a pattern: literal, ( pattern, ... ), identifier, or _.
 */
ASTNode* parse_pattern(Parser* p) {
    if (peek(p) == TOKEN_LPAREN) {
        ...
        return (ASTNode*)pat;
    } else if (peek(p) == TOKEN_IDENTIFIER) {
...
ASTNode* parse_while_stmt(Parser* p) {
    ...
    if (!match(p, TOKEN_WHILE)) {
        errhandler__report_error(p->line, p->column, "parser", "Expected 'while'");
        p->panic_mode = true;
        free(label);
        return NULL;
    }
...
ASTNode* parse_do_while_stmt(Parser* p) {
    ...
    if (!match(p, TOKEN_DO)) {
        errhandler__report_error(p->line, p->column, "parser", "Expected 'do'");
        p->panic_mode = true;
        free(label);
        return NULL;
    }
...
ASTNode* parse_for_stmt(Parser* p) {
    ...
    if (!match(p, TOKEN_FOR)) {
        errhandler__report_error(p->line, p->column, "parser", "Expected 'for'");
        p->panic_mode = true;
        free(label);
        return NULL;
    }
...
ASTNode* parse_receive_case(Parser* p) {
    ...
    if (match(p, TOKEN_PIPE)) {
        while (peek(p) == TOKEN_IDENTIFIER) {
            char* cap = strdup_custom(p->tokens[p->current].value);
            advance(p);
