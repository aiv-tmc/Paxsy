...
void parse_number(Lexer* lexer) {
    ...
    while (lexer->position < lexer->length) {
        if (CHARACTER(lexer, 0) == '.') {
            if (is_real) report_error(lexer->line, lexer->column, "Duplicate decimal point");
            is_real = true;
        } else if (CHARACTER(lexer, 0) == 'e') {
            if (has_exponent) report_error(lexer->line, lexer->column, "Duplicate exponent");
            is_real = true;
            has_exponent = true;
        } else if (CHARACTER(lexer, 0) == '_') {
            SHIFT(lexer, 1);
            continue;
        } else if (!isdigit(CHARACTER(lexer, 0))) break;
...
void tokenize(Lexer* lexer) {
    while (lexer->position < lexer->length) {
        ...
        case '#':
            ...
            while (lexer->position < lexer->length && isdigit(NEXT(lexer, 0)) {
                SHIFT(lexer, 1);
            }
            int base_len = lexer->position - base_start;
            add_token(lexer, TOKEN_INT, lexer->input + base_start, base_len);
            
            if (NEXT(lexer, 0) != ':') {
                report_error(lexer->line, lexer->column, "Expected ':' after base");
                return;
            }
            add_token(lexer, TOKEN_COLON, ":", 1);
            SHIFT(lexer, 1);
            
            int num_start = lexer->position;
            bool is_real = false;
            bool has_exponent = false;
            ...
            while (lexer->position < lexer->length) {
                if (NEXT(lexer, 0) == '.') {
                    ...
                    if (has_exponent) report_error(lexer->line, lexer->column, "Duplicate exponent");
                    has_exponent = true;
                } else if (NEXT(lexer, 0) == '_') {
                    SHIFT(lexer, 1);
                    continue;
                } else if (!isalnum(NEXT(lexer, 0)) && NEXT(lexer, 0) != '-' && NEXT(lexer, 0) != '+') break;
