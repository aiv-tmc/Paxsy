...
const char* token_names[] = {
    ...
    [TOKEN_PREPROC_PROGRAM]     =   "PREPROC_PROGRAM",
...
static bool is_valid_modifier(const char* modifier, uint32_t length) {
    const char* valid_modifiers[] = { "const", "unsigned", "signed", "extern", "static", "regis" };
        ...
        switch (current) {
            ...
            case 'N':
                if (strncmp(lexer->input + lexer->position, "NONE", 4) == 0) {
                    add_token(lexer, TOKEN_NONE, "NONE", 4); SHIFT(lexer, 4);
            ...
            case '=':
                ...
                break;
            case '_':
                    add_token(lexer, TOKEN_UNDERSCORE, "_", 1); SHIFT(lexer, 1);
                break;
            case '#':
                add_token(lexer, TOKEN_SHARP, "#", 1); SHIFT(lexer, 1);
                if (strncmp(lexer->input + lexer->position, "org", 3) == 0) {
                    add_token(lexer, TOKEN_PREPROC_ORG, "org", 3); SHIFT(lexer, 3);
                } else if (strncmp(lexer->input + lexer->position, "program", 7) == 0) {
                    add_token(lexer, TOKEN_PREPROC_PROGRAM, "program", 7); SHIFT(lexer, 7);
