...
static Type *parse_type_specifier(void) {
    ...
    if (!type) {
        REPORT_PARSE_ERROR("Memory allocation failed for type specifier");
        return NULL;
    }
    ...
    while (CURRENT_TOKEN_TYPE_MATCHES(TOKEN_MODIFIER) && type->modifier_count < MAX_MODIFIERS) {
        ...
        if (!new_modifiers) {
            REPORT_PARSE_ERROR("Memory allocation failed for modifiers");
        ...
        if (!type->modifiers[type->modifier_count]) {
            REPORT_PARSE_ERROR("Memory allocation failed for modifier string");
            for (int i = 0; i < type->modifier_count; i++) {
                free(type->modifiers[i]);
            }
            free(type->modifiers);
    ...
    while (CURRENT_TOKEN_TYPE_MATCHES(TOKEN_STAR) || CURRENT_TOKEN_TYPE_MATCHES(TOKEN_AMPERSAND)) {
        if (CURRENT_TOKEN_TYPE_MATCHES(TOKEN_STAR)) type->is_pointer = 1;
    ...
    if (CURRENT_TOKEN_TYPE_MATCHES(TOKEN_NUMBER)) {
        ...
        if (value < 0 || value > 255) {
            REPORT_PARSE_ERROR("Number out of range (0-255)");
            free_type(type);
            return NULL;
        }
    ...
    if (get_current_token_type() != TOKEN_TYPE) {
        REPORT_PARSE_ERROR("Expected type specifier");
        free_type(type);
        return NULL;
    }
...
static ASTNode *parse_block_statement(void) {
    if (CURRENT_TOKEN_TYPE_MATCHES(TOKEN_LCURLY)) {
        ...
        return block_node;
    } else if (CURRENT_TOKEN_TYPE_MATCHES(TOKEN_THEN)) {
        advance_token();
        ASTNode *statement = parse_statement();
    ...
    switch (node->type) {
        case AST_VARIABLE_DECLARATION:
            ...
            printf("sigil: %c\n", 
                ...
                node->declaration_sigil == TOKEN_STAR ? '*' : '&');
            for (int i = 0; i < indent+1; i++) printf("  ");
            printf("name: %s\n", node->value);
            if (node->variable_type) {
                ...
                printf("\n");
                if (node->variable_type->is_pointer) {
                    for (int i = 0; i < indent+1; i++) printf("  ");
                    printf("PTR\n");
                }
        ...
        case AST_UNARY_OPERATION:
            printf("UnaryOp: %s", token_names[node->operation_type]);
            print_ast_node(node->right, indent + 1);
            break;
...
static ASTNode *parse_int(void) {
    CONSUME_TOKEN(TOKEN_INT);
    expect_token(TOKEN_DOUBLE_COLON);
    
    if (!CURRENT_TOKEN_TYPE_MATCHES(TOKEN_NUMBER)) {
        REPORT_PARSE_ERROR("Expected int number after '::'");
        return NULL;
    }
    
    Token *number_token = get_current_token();
    char *end_pointer;
    long int_value = strtol(number_token->value, &end_pointer, 0);
    if (*end_pointer != '\0') {
        REPORT_PARSE_ERROR("Invalid number format for int");
        return NULL;
    }
    char int_number_string[20];
    snprintf(int_number_string, sizeof(int_number_string), "%ld", int_value);
    char *int_number = strdup(int_number_string);
    advance_token();
    ...
    if (CURRENT_TOKEN_TYPE_MATCHES(TOKEN_LPAREN)) {
        ...
        while (!CURRENT_TOKEN_TYPE_MATCHES(TOKEN_RPAREN) && !CURRENT_TOKEN_TYPE_MATCHES(TOKEN_EOF)) {
            if (arguments->count >= MAX_INT_ARGS) {
                REPORT_PARSE_ERROR("Too many arguments for int, maximum is %d", MAX_INT_ARGS);
                free_ast(arguments);
                free(int_number);
                return NULL;
            }
    ...
    return create_ast_node(AST_INT, 0, int_number, (ASTNode*)arguments, NULL, NULL);
}
...
static ASTNode *parse_parameter_list(bool *is_variadic) {
    ...
    *is_variadic = false;
    ...
    while (get_current_token_type() != TOKEN_RPAREN && get_current_token_type() != TOKEN_EOF) {
        ...
        if (!CURRENT_TOKEN_TYPE_MATCHES(TOKEN_DOLLAR) && !CURRENT_TOKEN_TYPE_MATCHES(TOKEN_STAR) && !CURRENT_TOKEN_TYPE_MATCHES(TOKEN_AMPERSAND)) {
...
static ASTNode *parse_function_declaration(void) {
    ...
    else if (ATTEMPT_CONSUME_TOKEN(TOKEN_THEN)) {
        ASTNode *statement = parse_statement();
...
static ASTNode *parse_label_declaration(void) {
    CONSUME_TOKEN(TOKEN_TILDE);
    
    if (!CURRENT_TOKEN_TYPE_MATCHES(TOKEN_ID)) {
        REPORT_PARSE_ERROR("Expected label name");
        return NULL;
    }
    Token *token = get_current_token();
...
static ASTNode *parse_label_call(void) {
    Token *token = get_current_token();
    char *label_name = strdup(token->value);
    advance_token();
    CONSUME_TOKEN(TOKEN_SEMICOLON);
    return create_ast_node(AST_LABEL_CALL, 0, label_name, NULL, NULL, NULL);
}
...
static ASTNode *parse_assignment_expression(void) {
    ...
    if (CURRENT_TOKEN_TYPE_MATCHES(TOKEN_EQUAL) || CURRENT_TOKEN_TYPE_MATCHES(TOKEN_PLUS_EQ) || CURRENT_TOKEN_TYPE_MATCHES(TOKEN_MINUS_EQ) ||
        CURRENT_TOKEN_TYPE_MATCHES(TOKEN_STAR_EQ) || CURRENT_TOKEN_TYPE_MATCHES(TOKEN_SLASH_EQ) || CURRENT_TOKEN_TYPE_MATCHES(TOKEN_PERCENT_EQ) ||
        CURRENT_TOKEN_TYPE_MATCHES(TOKEN_DOUBLE_STAR_EQ) || CURRENT_TOKEN_TYPE_MATCHES(TOKEN_PIPE_EQ) || CURRENT_TOKEN_TYPE_MATCHES(TOKEN_AMPERSAND_EQ) ||
        CURRENT_TOKEN_TYPE_MATCHES(TOKEN_CARET_EQ) || CURRENT_TOKEN_TYPE_MATCHES(TOKEN_TILDE_EQ) || CURRENT_TOKEN_TYPE_MATCHES(TOKEN_SHL_EQ) ||
        CURRENT_TOKEN_TYPE_MATCHES(TOKEN_SHR_EQ) || CURRENT_TOKEN_TYPE_MATCHES(TOKEN_SAL_EQ) || CURRENT_TOKEN_TYPE_MATCHES(TOKEN_SAR_EQ) ||
        CURRENT_TOKEN_TYPE_MATCHES(TOKEN_ROL_EQ) || CURRENT_TOKEN_TYPE_MATCHES(TOKEN_ROR_EQ)) {
...
static ASTNode *parse_unary_expression(void) {
    if (CURRENT_TOKEN_TYPE_MATCHES(TOKEN_PLUS) || CURRENT_TOKEN_TYPE_MATCHES(TOKEN_MINUS) || CURRENT_TOKEN_TYPE_MATCHES(TOKEN_BANG) || 
        CURRENT_TOKEN_TYPE_MATCHES(TOKEN_TILDE) || CURRENT_TOKEN_TYPE_MATCHES(TOKEN_AMPERSAND) ||
        CURRENT_TOKEN_TYPE_MATCHES(TOKEN_DOUBLE_PLUS) || CURRENT_TOKEN_TYPE_MATCHES(TOKEN_DOUBLE_MINUS) || CURRENT_TOKEN_TYPE_MATCHES(TOKEN_STAR) || CURRENT_TOKEN_TYPE_MATCHES(TOKEN_DOUBLE_STAR) || CURRENT_TOKEN_TYPE_MATCHES(TOKEN_SLASH)) {
        TokenType operation = get_current_token_type();
        advance_token();
...
static ASTNode *parse_statement(void) {
    ...
    switch (token->type) {
        ...
        default: {
            if (CURRENT_TOKEN_TYPE_MATCHES(TOKEN_ID)) {
                Token *next_token = get_current_token() + 1;
                if (next_token < token_stream + total_tokens && next_token->type == TOKEN_SEMICOLON) {
                    return parse_label_call();
                }
            }
            ASTNode *expression = parse_expression();
            if (expression) {
